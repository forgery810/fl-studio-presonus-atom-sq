...
# of the Plugin exactly. Within each plugin dict, there is a list of parameters the touchpad will change. It must follow this format:
# [number of parameter, value at 0 , value at 127] Value at 0 refers to what you want the value of that parameter to be when the touchpad is 
# fully left and 127 is the value outputted when the touch pad is the right. 0 is the minimum and 1.0 is the max. Look at the examples below for guidance.
# The line below can be deleted or can have a # put in front to deactivate them. 

touchpad_params = {
			'Transistor Bass': [[tb['Cutoff'], 1, .5], [tb['Waveform'], 0, .5], [tb['Resonance'], .5, 1], [tb['Envelope mod'], 0, 1]],
